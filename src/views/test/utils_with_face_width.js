 ...
 * @param {*} points
 */
export const getMedian = points => points[Math.floor(points.length / 2)]
 ...
 * @param {*} pb
 */
export const getMidPoint = (pa, pb) => ({
  x: (pa.x + pb.x) / 2,
  y: (pa.y + pb.y) / 2
})
 ...
 * @param {*} rightPoints
 */
export const getMidPointOfEyebrows = (leftPoints, rightPoints) =>
 ...
 * @param {*} jawPoints
 */
export const getJawPos = jawPoints => getMedian(jawPoints)
 ...
 */
export const getFaceLength = (jawPos, midPointOfEyebrows) => (5 * getDistance(jawPos, midPointOfEyebrows)) / 3

export const distanceOfPoint = (point, a, b, c) => Math.abs(a * point.y + b * point.x + c) / Math.sqrt(a * a + b * b)

/**
 * 获取脸的宽度（即帽子宽度）
 * @param {*} outlinePoints
 */
export const getFaceWith = (outlinePoints, jawPos, midPointOfEyebrows) => {
  const k = getK(jawPos, midPointOfEyebrows)
  const b = jawPos.x - k * (jawPos.y)

  const leftWidth = distanceOfPoint(outlinePoints[0], k, -1, b)
  const rightWidth = getDistance(outlinePoints[outlinePoints.length - 1], k, -1, b)
 ...
 * @param {*} midPointOfEyebrows
 */
export const getFaceRadian = (jawPos, midPointOfEyebrows) =>
 ...
 * @param {*} point
 */
export const getPos = (k, d, point) => {
  // 取y变小的那一边
  const y = -Math.sqrt((d * d) / (1 + k * k)) + point.y
  const x = k * (y - point.y) + point.x
  return { x, y }
}
 ...
 * @param {*} jawPos 下巴底点坐标
 */
export const getHeadPos = (midPos, jawPos) => {
  // 获取线的k值
 ...
 * @param {*} b
 */
export const getK = (a, b) => (a.x - b.x) / (a.y - b.y)
